...
const checkShipOnBoard = (ship: Warship) => {
    switch (ship.orientation) {
        case Orientation.HORIZONTAL:
            if (ship.position.x + ship.length - 1 >= Brush.FIELD_COUNT || ship.position.x <0) {
                ship.position = initialField
            }
            break;

        case Orientation.VERTICAL:
            if (ship.position.y + ship.length - 1 >= Brush.FIELD_COUNT || ship.position.y <0) {
                ship.position = initialField
            }
            break;
